...
class MyList {
	...
	public int getSecondMaxItem() {
		...
		while (iterator.hasNext()) {
			...
			if (maxItem <= temp)
				secondMaxItem = Math.max(secondMaxItem, temp);
			// if (maxItem != temp) secondMaxItem=Math.max(secondMaxItem,temp); Changed so
			// thaht if there is a tie for max, 2nd highest is that element.
		}
	...
	/*
	 * public void printMyList() { Iterator<Integer> iterator = myList.iterator();
	 * while (iterator.hasNext()) System.out.print(iterator.next() + " ");
	 * System.out.println(""); }
	 * 
	 * public void addItems(Integer[] array) { for (int item : array) {
	 * this.myList.add(item); } }
	 */
